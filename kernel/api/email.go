...
var emailAccounts = make(map[string]*imap.Dialer)

func loginEmailAccount(c *gin.Context) {
	...
	account_alias := arg["account_alias"].(string)
	imap_addr := arg["imap_addr"].(string)
	imap_port_string := arg["imap_port"].(string)
	username  := arg["username"].(string)
	passwd  := arg["passwd"].(string)
	...
	im, err := imap.New(username, passwd, imap_addr, imap_port)
	if err != nil {
		ret.Code = -1
		ret.Msg = err.Error()
		return
	...
	emailAccounts[account_alias] = im
	ret.Msg = "Successfully login in!"
}
